...
describe('#lenqueue/ldequeue/lremove/lget', function() {
    ...
    it('should succ to lenqueue new key, get/dequeue to existed key', function(done) {
        var cb = H.client;
        var key = H.genKey("queue_key2");
        ...
        cb.remove(key, function(){
            cb.lenqueue(key, value, function(err, result) {
                ...
                cb.lget(key, function(err, result) {
                    assert.equal(err, undefined, "should not return err: " + err);
                    assert.equal(result.value, [value], "should get same:", result.value, "!=", [value]);
                    cb.ldequeue(key, function(err, result){
                        assert.equal(err, undefined, "should not return err: " + err);
                        assert.equal(result.value, value, "should dequeue same:", result.value, "!=", value);
                        cb.lget(key, function(err, result) {
                            assert.equal(err.code, H.errors.keyNotFound);
                            done();
                        });
    ...
    it('should succ to lenqueue old key, get/dequeue to existed key', function(done) {
        var cb = H.client;
        var key = H.genKey("queue_key3");
        var value = bignum("FFFEFDFCF0F1F2F3", 16);
        cb.lenqueue(key, value, function(err, result) {
            assert.equal(err, undefined, "should not return err: " + err);
            cb.lenqueue(key, value, function(err, result) {
                ...
                cb.lget(key, function(err, result) {
                    assert.equal(err, undefined, "should not return err: " + err);
                    assert.equal(result.value, [value, value], "should get same:", result.value, "!=", [value, value]);
                    cb.ldequeue(key, function(err, result){
                        assert.equal(err, undefined, "should not return err: " + err);
                        assert.equal(result.value, value, "should dequeue same:", result.value, "!=", value);
                        cb.ldequeue(key, function(err, result){
    ...
    it('should succ to lenqueue new key, get/remove to existed key', function(done) {
        var cb = H.client;
        var key = H.genKey("queue_key4");
        var value = bignum("FFFEFDFCF0F1F2F3", 16);
        cb.remove(key, function(){
            cb.lenqueue(key, value, function(err, result) {
                assert.equal(err, undefined, "should not return err: " + err);
            ...
            cb.lenqueue(key, value2, function(err, result) {
                ...
                cb.lenqueue(key, value1, function(err, result) {
                    ...
                    cb.lremove(key, value3, function(err, result){
                        ...
                        cb.lget(key, function(err, result) {
                            assert.equal(err, undefined, "should not return err: " + err);
                            assert.equal(result.value, [value1, value2, value1], "should get same:", 
                            ...
                            cb.lremove(key, value1, function(err, result){
                                ...
                                cb.lget(key, function(err, result) {
                                    assert.equal(err, undefined, "should not return err: " + err);
                                    assert.equal(result.value, [value2], "should get same:", 
                                        result.value, "!=", [value2]);
                                    // remove value1 -> []
