...
function Connection(options, callback) {
  ...
  if (ourObjs.mchost !== undefined) {
    this.mchosts = ourObjs.mchosts;
    delete ourObjs.mchosts;
  } else {
    this.mchosts = null;
  }

  if (ourObjs.srvhost !== undefined) {
    this.srvhost = ourObjs.srvhost;
    delete ourObjs.srvhost;
  } else {
    this.srvhost = null;
  }
  ...
  // save of the bucket name
  this._bucket = cbArgs[3];
  ...
  try {
    this._cb = new CBpp('__TEMPORARYHOST', cbArgs[1], cbArgs[2], cbArgs[3], cbArgs[4]);
  } catch (e) {
    callback(e);
  }
  ...
  // Actual connect handler
  var performConnect = function __performConnect() {
    if (!this.hosts) {
      this.hosts = '';
    }
    if (!this.mchosts) {
    ...
    this._cb._control.call(this._cb, CONST.LCB_CNTL_CONFIG_HTTP_NODES, CONST.LCB_CNTL_SET, this.hosts);
    this._cb._control.call(this._cb, CONST.LCB_CNTL_CONFIG_CCCP_NODES, CONST.LCB_CNTL_SET, this.mchosts);
  ...
  if (!this.srvhost) {
    ...
    dns.resolveSrv('_cbmcd._tcp.' + this.srvhost, function(err, mcdaddrs) {
      if (err) mcdaddrs = [];

      dns.resolveSrv('_cbhttp._tcp.' + this.srvhost, function(err, httpaddrs) {
        if (err) httpaddrs = [];

        var mcdhosts = [];
        for (var i = 0; i < mcdaddrs.length; ++i) {
          mcdhosts.push(mcdaddrs[i].name + ':' + mcdaddrs[i].port);
        }
        if (mcdhosts.length > 0) {
          this.hosts = mcdhosts.join(';');
        }

        var httphosts = [];
        for (var j = 0; j < httpaddrs.length; ++j) {
          httphosts.push(httpaddrs[j].name + ':' + httpaddrs[j].port);
        }
        if (httphosts.length > 0) {
          this.mchosts = httphosts.join(';');
        }
...
Connection.prototype._handleRestResponse = function(error, response, callback) {
  ...
  if (response.data === undefined) {
    // we don't have a body so we are done here, if this is an error it must be
    // handled in further down the call chain
 ...
 * });
 */
Connection.prototype.lenqueue = function(key, value, option, callback) {
    var val = bignum(value);
    var buf = new Buffer(8);
    buf.writeUInt32BE(val.shiftRight(32), 0);
    buf.writeUInt32BE(val.and(0xFFFFFFFF), 4);
    arguments[1] = buf;
    this._invokeStorage(this._cb.lenqueueMulti, arguments);
};

Connection.prototype.lremove = function(key, value, option, callback) {
    var val = bignum(value);
    var buf = new Buffer(8);
    buf.writeUInt32BE(val.shiftRight(32), 0);
    buf.writeUInt32BE(val.and(0xFFFFFFFF), 4);
    arguments[1] = buf;
    this._invokeStorage(this._cb.lremoveMulti, arguments);
};

Connection.prototype.ldequeue = function(key, options, callback) {
    if (arguments.length > 1) {
        callback = arguments[arguments.length-1];
        arguments[arguments.length -1] = function(err, result) {
            if(!err && result.value.length === 8) {
                var higher = result.value.readUInt32BE(0);
                var lower = result.value.readUInt32BE(4);
                result.value = bignum(higher).shiftLeft(32).or(lower);
            }
            callback(err, result);
        };
    }

    this._argHelper2(this._cb.ldequeueMulti, arguments);
};

Connection.prototype.lget = function(key, options, callback) {
    if (arguments.length > 1) {
        callback = arguments[arguments.length-1];
        arguments[arguments.length -1] = function(err, result) {
            if(!err && result.value.length%8 === 0) {
                var buffer = result.value;
                result.value = [];
                var offset = 0;
                while (offset < buffer.length) {
                    var higher = buffer.readUInt32BE(offset);
                    var lower = buffer.readUInt32BE(offset + 4);
                    offset += 8;
                    result.value.push(bignum(higher).shiftLeft(32).or(lower));
                }
            }
            callback(err, result);
        };
    }
    this._argHelper2(this._cb.lgetMulti, arguments);
};

Connection.prototype.lgetMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.lgetMulti, arguments);
};

Connection.prototype.sadd = function(key, value, option, callback) {
    var val = bignum(value);
    var buf = new Buffer(8);
    buf.writeUInt32BE(val.shiftRight(32), 0);
    buf.writeUInt32BE(val.and(0xFFFFFFFF), 4);
    arguments[1] = buf;
    this._invokeStorage(this._cb.saddMulti, arguments);
};

Connection.prototype.sremove = function(key, value, option, callback) {
    var val = bignum(value);
    var buf = new Buffer(8);
    buf.writeUInt32BE(val.shiftRight(32), 0);
    buf.writeUInt32BE(val.and(0xFFFFFFFF), 4);
    arguments[1] = buf;
    this._invokeStorage(this._cb.sremoveMulti, arguments);
};

Connection.prototype.sismember = function(key, value, option, callback) {
    var val = bignum(value);
    var buf = new Buffer(8);
    buf.writeUInt32BE(val.shiftRight(32), 0);
    buf.writeUInt32BE(val.and(0xFFFFFFFF), 4);
    arguments[1] = buf;
    this._invokeStorage(this._cb.sismemberMulti, arguments);
};

Connection.prototype.sget = function(key, options, callback) {
    if (arguments.length > 1) {
        callback = arguments[arguments.length-1];
        arguments[arguments.length -1] = function(err, result) {
            if(!err && result.value.length%8 === 0) {
    ...
    this._argHelper2(this._cb.sgetMulti, arguments);
};

Connection.prototype.sgetMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.sgetMulti, arguments);
};
 ...
 * @see Connection#prepend
 */
Connection.prototype.lenqueueMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.lenqueueMulti, arguments);
};

Connection.prototype.lremoveMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.lremoveMulti, arguments);
};

Connection.prototype.ldequeueMulti = function(kv, options, callback) {
  this._multiHelper(this._cb.ldequeueMulti, arguments);
};
...
Connection.prototype.saddMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.saddMulti, arguments);
};

Connection.prototype.sremoveMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.sremoveMulti, arguments);
};

Connection.prototype.sismemberMulti = function(kv, options, callback) {
    this._multiHelper(this._cb.sismemberMulti, arguments);
};
...
/**
 * Deletes all the documents from the current bucket.
 *
 * @param {FlushCallback} callback
 */
Connection.prototype.flush = function(callback) {
  this._cb.httpRequest.call(this._cb, null, {
    path: '/pools/default/buckets/' + this._bucket + '/controller/doFlush',
    content_type: 'application/json',
    method: CONST.LCB_HTTP_METHOD_POST,
    lcb_http_type: CONST.LCB_HTTP_TYPE_MANAGEMENT
  }, callback);
};
